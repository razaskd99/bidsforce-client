...
export default async function AdminPanelDesignation({searchParams}) {
  ...
  let records = await getAllOppPrereqRecordsAction("business_line", searchTermValue, offset, limit);
  const allRecords = records?.returnData?.data || [];
  const total_count = records?.returnData?.total_count || 0;
  const totalPages = Math.ceil(total_count / limit) || 0

  const breadcrumbItems = [
    { label: "Home", href: "/admin-panel" },
    { label: "Business Line", href: "/admin-panel/opportunity/business-line" },
  ];
  ...
  return (
    <div className=" w-full">
      ...
      <div>
        <OppPrereqTable allRecords={allRecords} tableName={"business_line"}/>
      </div>
