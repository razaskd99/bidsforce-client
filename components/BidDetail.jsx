...
import { updateRfxStatuAction } from "@/app/api/manager/actions/closing";
...
const BidDetail = ({
  ...
  const [showSubmissionTempSelect, setshowSubmissionTempSelect] =
    useState(false);
  ...
  const [selectedClarificationContact, setSelectedClarificationContact] =
    useState(null);
  const [showSubmitedRfxCalrification, setShowSubmitedRfxCalrification] =
    useState(false);
  const [showBidClarificationDetial, setShowBidClarificationDetial] =
    useState(false);
  ...
  const [rfxClarifRefNum, setRfxClarifRefNum] = useState("");
  ...
  const [closingOrderDetails, setClosingOrderDetails] = useState(
    bidOrderRec.length > 0 ? bidOrderRec[0] : {}
  );
  ...
  const [overviewData, setoverviewData] = useState([
    ...
    {
      name: "Existed supplier Agreement",
      value: rfxRecord.agreement_ref_num ? "Yes" : "No",
    },
    ...
    {
      name: "Visit to worksite",
      value: rfxRecord.visit_worksite ? "Required" : "Not Required",
    },
    {
      name: "Other instructions for  site visit",
      value: rfxRecord.visit_worksite_instructions,
    },
  ...
  const [selectedDetailTemplateQuestions, setSelectedDetailTemplateQuestions] =
    useState({});
  ...
  const handleRowClick = async (rowId) => {
    ...
    if (active == "Detailed Review") {
      // get selected row
      const targetRow = detailedReviewRec.find(
        (item) => item.bid_review_id === rowId
      );
      setSelectedReviewRow(targetRow);
      // get comment list
    ...
    if (active == "Final Review") {
      // get selected row
      const targetRow = finalReviewRec.find(
        (item) => item.bid_review_id === rowId
      );
      setSelectedReviewRow(targetRow);
      // get comment list
    ...
    if (active == "RFx Clarifications") {
      ...
      setRfxClarPostRows(r3.returnData);
      // get rfx clarif post documents
      const r4 = await GetRfxDocumentsAction(rfxRecord.rfx_id);
      setRfxClarPostDocsRows(r4.returnData);
      // get rfx clarif contacts
  ...
  const handleChangeStatus = async (stageName) => {
    ...
    if (
      stageName == "Pre-lim Review" &&
      stages[currentIndex].stage == "Initiate"
    ) {
      let r1 = {};
      selectedContact &&
        selectedContact.map(
    ...
    if (currentIndex !== -1) {
      ...
      if (currentIndex < stages.length - 1) {
        ...
        if (stages[currentIndex].stage == "Initiate") {
          ...
          let c3 = await changeRfxStatusJSON(c2?.rfxData?.status, stages[currentIndex].displayName);
  ...
  async function changeRfxStatusJSON(jsonString, status) {
    try {
      ...
      const json = {...jsonObject, bid: status};
  ...
  const addPreLimRow = async () => {
    ...
    if (response.statusCode == 200) {
      ...
      selectedUsers &&
        selectedUsers.map(
          async (item, index) =>
            (r2 = await createReviewContactsAction(
              result.bid_review_id,
              item.id,
              item.role,
              false,
              ""
      ...
      // get reviews list
      r2 = await getAllReviewsRecordsBy_Rfx_Key_Action(
        rfxRecord.rfx_id,
        "pre-lim-review"
      );
  ...
  const addDetailRevRow = async () => {
    ...
    if (response.statusCode == 200) {
      let r2 = {};
      // add contacts
      selectedDetailsUsers &&
        selectedDetailsUsers.map(
          async (item, index) =>
            (r2 = await createReviewContactsAction(
              result.bid_review_id,
              item.id,
  ...
  const addRfxClarRow = async () => {
    ...
    if (r1.statusCode == 200 && selectedContact.length) {
      ...
      for (var i = 0; i < selectedContact.length; i++) {
        let r3 = await createContactsAction(
          rfxRecord.rfx_id,
          selectedContact[i].user_id,
          "rfx-clarifications-" + clarif_id
        );
      }
    }
    ...
    let r3 = await getAllRfxClarificationRecordsBy_RfxID_Action(
      rfxRecord.rfx_id
    );
  ...
  useEffect(() => {
    if (active === "Submission") {
      // get submission list
      getAllSubmissionAction(rfxRecord.rfx_id)
        .then((res) => {
            if (res.statusCode == 200) {
                let subRec = res.returnData;
  ...
  useEffect(() => {
    if (active === "Bid Clarifications") {
      // get bid clarif list
      getAllBidClarificationRecordsBy_RfxID_Action(rfxRecord.rfx_id)
        .then((res) => {
  ...
  const addSubmissionRow = async () => {
    showMainLoader102();
    setSubmissionDetail(false);
    setShowSubmissionTable(true);
    setshowSubmissionTempSelect(false);
    //const newRow = { id: finalRevrows.length + 1, checkbox: 'dddd.png', description: 'New Opportunity', crmId: 'newCrmId', customer: 'New Customer', issueDate: 'New Date', salePerson: 'New Salesperson', status: 'New Status', };
  ...
  const handleBidClarifReplySubmit = async () => {
    ...
    if (rowIdSelected && rowIdSelected > 0) {
      ...
      // get bid carification list
      const r2 = await getAllBidClarificationRecordsBy_RfxID_Action(rfxRecord.rfx_id)
      const recList = r2.returnData
      setBidClarifRows(recList)
  ...
  const handleCloseOrder = async()=>{
    const r1 = await updateRfxStatuAction(rfxRecord.rfx_id, 'Closed') 
    if(r1.statusCode == 200){
        handleChangeStatus();
    ...
    <Stack height="100%" alignItems="center" justifyContent="center">
      ...
      <div className="flex items-center gap-6">
        <button
          ...
          onClick={handleClarificationDetail}
        >
  ...
  const NoRowsOverlayOrder = () => (
    <Stack height="100%" alignItems="center" justifyContent="center">
      ...
      <p className="text-[#778CA2] text-lg font-light mb-3">
        You can see order requests here
      </p>
      {/*<div className="flex items-center gap-6">
  ...
  const NoRowsPrelimReview = () => (
    <Stack height="100%" alignItems="center" justifyContent="center">
      ...
      <p className="text-[#778CA2] text-lg font-light mb-3">
        You have not created any review/approval requests
      </p>
      <div className="flex items-center gap-6">
        <button
          className="text-white bg-[#26BADA] p-3 uppercase  min-w-[200px]"
          onClick={() => {
            handleshowTemplateSelect();
          }}
        >
          Request Review
        </button>
      ...
      <SkipDialog
        ...
        updateParentSkipReason={updateParentSkipReason}
      />
    </Stack>
  );
    ...
    <Stack height="100%" alignItems="center" justifyContent="center">
      ...
      <div className="flex items-center gap-6">
        <button
          className="text-white bg-[#26BADA] p-3 uppercase  min-w-[200px]"
          onClick={() => {
            handleshowFinalTemplateSelect();
            setShowFinalRewList(false);
            setInFinalTemp(false);
          }}
        >
          Request Review
        </button>
  ...
  const NoRowsOverlayBidClarification = () => (
    <Stack height="100%" alignItems="center" justifyContent="center">
      ...
      <p className="text-[#252631] text-xl mb-3">No requests yet</p>
      <p className="text-[#778CA2] text-lg font-light mb-3">
        You have no Bid Clarification request
      </p>
      {/*<div className="flex items-center gap-6">
    ...
    <Stack height="100%" alignItems="center" justifyContent="center">
      ...
      <div className="flex items-center justify center">
        <button
          ...
          onClick={() => {
            setshowSubmissionTempSelect(true);
            setSubmissionTemplate(false);
            setSubmissionDetail(false);
            setShowSubmissionTable(false);
            setInSubmissionTemp(false);
          }}
        >
          Submit Bid
        </button>
  ...
  const NoRowsRevision = () => (
    <Stack height="100%" alignItems="center" justifyContent="center">
      ...
      <p className="text-[#778CA2] text-lg font-light mb-3">
        You have no any revision requests
      </p>
    </Stack>
  );
  ...
  const NoRowsOverlayOrderHandover = () => (
    <Stack height="auto" alignItems="center" justifyContent="center">
      ...
      <p className="text-[#778CA2] text-lg font-light mb-3">
        The team has been notified about the order transfer
      </p>
    </Stack>
  );
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between gap-5">
        ...
        <div className="flex bg-white p-5 justify-between items-center w-[25%] shadow-sm">
          ...
          <div className="flex flex-col flex-[3]">
            ...
            <span className="text-sm text-[#98A9BC]">
              <span className="text-black"> 29 </span>days{" "}
              <span className="text-black"> 8 </span> hours
            </span>
          </div>
        </div>
        <div className="flex bg-white p-5 justify-between items-center w-[25%] shadow-sm">
      ...
      <div className="Content bg-white">
        {(active === "Overview" || active === "Initiate") && (
                  ...
                  docvaltRec.map((doc) => (
                    <div className=" shadow-sm flex items-center w-full p-2 justify-between mb-3 border-b border-[#babec2]">
                      ...
                      <span className="text-[#98A9BC]">
                        {formatDateString(doc.created_at)}
                      </span>
              ...
              <div className="flex-[1] flex flex-col">
                ...
                <div className="border mt-[18px] mb-3 rounded-md">
                  ...
                  <div className="bg-[#F4FCFD] px-4 py-5">
                    ...
                    <span>
                      {formatDateString(rfxRecord.tech_clarification_deadline)}
                    </span>
                  </div>
                  <div className="border-b border-[#E8ECEF] w-[90%] m-auto"></div>
                  <div className="bg-[#F4FCFD] px-4 py-5">
                  ...
                  <div className="bg-[#F4FCFD] px-4 py-5">
                    ...
                    <span>
                      {rfxRecord.expected_award_date
                        ? formatDateString(rfxRecord.expected_award_date)
                        : "No Date"}{" "}
                    </span>
                  </div>
                </div>
                <div className="border mb-3 rounded-md">
                  ...
                  <div className="bg-[#F4F5F6] py-3 px-4 flex   items-center justify-between">
                    <AssignPopupInput
                      label={"Assign to"}
                      allUsersRec={allUsersRec}
                      rfx_bid_assignto={rfxRecord.rfx_bid_assignto}
                ...
                <div>
                  <div className="border mb-3 rounded-md">
                    <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-between">
                      <p>Bid Team</p>
                      <p
                        className="uppercase text-[#26BADA] text-xs flex items-center gap-2 cursor-pointer"
                        ...
                        <div className="flex flex-[3] bg-white border rounded-[30px] p-1 gap-2 items-center max-w-[60%] w-full">
                          ...
                          <div className="">
                            ...
                            <span className="text-sm leading-4 text-[#778CA2] block">
                              {selectedContact.designation_title}
                            </span>
                          </div>
                        </div>
          ...
          <div className="p-5 bg-white shadow-sm h-full min-h-screen">
            {!showTemplateSelect && (
              <>
                <div className="flex justify-between uppercase text-[#00AAEC] text-sm mb-4 cursor-pointer ">
                  ...
                  <button
                    className={`upprecase uppercase max-w-[200px] flex items-center gap-1 rounded-md p-2  ${
                        preLimRows.length
                        ? "bg-[#00AAEC] text-white"
                        : "bg-[#EFF3F5] text-[#98A9BC]"
                    }`}
                    onClick={handleChangeStatus}
                    disabled={!preLimRows.length}
                  >
                ...
                <SearchTableNew
                  ...
                  NoRowsOverlay={NoRowsPrelimReview}
                />
              </>
            )}
            {showTemplateSelect && !loadTemplate && (
              <div className="flex flex-col gap-4 mt-5">
                <div
                  className="flex items-center gap-1text-[#26BADA] cursor-pointer mb-1"
                  onClick={() => {
                ...
                />
                <button
                  className={`uppercase p-[10px] max-w-[240px] ${
                    isButtonActive
              ...
              <>
                <div
                  ...
                  onClick={() => {
                    setshowTemplateSelect(
                      prelimReviewRec.length ? false : true
                    );
                    setloadTemplate(false);
                  }}
                >
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-[1] flex flex-col items-center p-3 gap-4">
                    {
                      <button
                        className={`text-center  py-3 mt-[10px] mb-[18px] rounded-md border-0 uppercase w-full ${
                          !activeSubmit
                            ? "bg-[#26BADA] text-white"
                            : "bg-[#EFF3F5] text-[#98A9BC]"
                        }`}
                    ...
                    <div className="rounded-md border border-gray-300 w-full mt-4 ">
                      ...
                      <div className="bg-[#F8FAFB] text-center p-8">
                        <p className="text-5xl text-[#778CA2]">
                          {selectedReviewRow?.score_value}:{" "}
                          <span className="text-[#6DD230]">
                            {selectedReviewRow?.score_name}
                          </span>
                        </p>
                        <p className=" text-[#778CA2] my-4">
                          {selectedReviewRow?.score_description
                            ? selectedReviewRow?.score_description
                            : "No Description"}{" "}
                        </p>
                      </div>
                    ...
                    <div className="rounded-md border border-gray-300 w-full">
                      <div className="bg-[#EFF3F5] text-[#778CA2]  flex justify-between p-3 items-center">
                        ...
                        {!viewTemplateDetails && (
                          <span
                            className="text-[#00AAEC] flex items-center text-sm gap-2 cursor-pointer"
                            onClick={handleReviewerDialogOpen}
                          >
                    ...
                    {inTemplateDetail && (
                      <div className="rounded-md border border-gray-300 w-full">
                        ...
                        <div className="bg-[#E8ECEF] p-2">
                          {reviewReplyCommentList &&
                            reviewReplyCommentList.map((item) => (
                              <div className="flex">
                                <div className="flex-[1]">
                                  <Image
                                    src="/avatar.jpg"
                                ...
                                <div className="flex flex-col flex-[5] p-2 mb-2">
                                  <div className="flex justify-between">
                                    ...
                                    <span className="text-sm text-[#26BADA]">
                                      {item.status}
                                    </span>
                                  </div>
                                  <p className="text-sm text-[#778CA2]">
                                    {item.comment}
                                  </p>
                                </div>
                              </div>
                          ...
                          <div className="block">
                            {
                              <>
                                <textarea
                                  ...
                                  onChange={(e) =>
                                    setReviewReplyComment(e.target.value)
                                  }
                                ></textarea>
                                <select
                                  id="rfx_type"
                                  ...
                                  onChange={(e) =>
                                    setReviewReplyStatus(e.target.value)
                                  }
                                >
          ...
          <div className="p-5 bg-white shadow-sm h-full min-h-screen">
            <div className="max-w-[80%]">
              <div className="flex justify-end items-center">
                <button
                  className={`upprecase my-1 uppercase rounded-md p-2  bg-[#00AAEC] text-white cursor-pointer flex items-center gap-1`}
                  onClick={handleChangeStatus}
                  disabled={false}
                >
                  Proceed <FaArrowRight />{" "}
              ...
              <ControlledAccordions
                ...
                allUsersRec={allUsersRec}
                koffRec={koffRec}
                deliverablesRec={deliverablesRec}
                login_user_id={login_user_id}
                bidteamRec={bidteamRec}
                active={active === "Bid Setup" ? true : false}
              />
            </div>
          </div>
        )}

        {active === "Detailed Review" && (
          <div className="p-5 bg-white shadow-sm h-full min-h-screen">
            {!showDetailTempSelect && (
              <>
                <div className="flex justify-between uppercase text-[#00AAEC] text-sm mb-4 cursor-pointer ">
                  <div
                    className="flex items-center gap-1"
                    onClick={addDetailRevRow}
                  >
                    <span onClick={handleshowDetTemplateSelect}>
                      {" "}
                      New Request
                    </span>
                    <IoMdAddCircleOutline />
                  ...
                  <button
                    className={`upprecase uppercase max-w-[200px] rounded-md p-2 flex items-center gap-1 ${
                      detailRevRow.length
                        ? "bg-[#00AAEC] text-white"
                        : "bg-[#EFF3F5] text-[#98A9BC]"
                    }`}
                    onClick={handleChangeStatus}
                    disabled={!detailRevRow.length}
                  >
                    Proceed <FaArrowRight />{" "}
                  </button>
                </div>
                <SearchTableNew
                  rows={detailRevRow}
                  handleRowClick={handleRowClick}
                  NoRowsOverlay={NoRowsDetailReview}
                />
              </>
            )}
            {showDetailTempSelect && !detailTemplate && (
              <div className="flex flex-col gap-4 mt-5">
                <div
                  className="flex items-center gap-1 cursor-pointer text-[#26BADA] mb-1"
                  onClick={() => {
                    setshowDetailTempSelect(false);
                    setloadTemplate(false);
                    setDetailTemplate(false);
                  }}
                >
                ...
                <button
                  className={`uppercase p-[10px] max-w-[240px] ${
                    isDetailButtonActive
                      ? "bg-[#26BADA] text-white"
                      : "bg-[#EFF3F5] text-[#98A9BC]"
                  }`}
                  disabled={!isDetailButtonActive}
                  onClick={() => {
                    setDetailTemplate(true);
                  }}
                >
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-[1] flex flex-col items-center p-3 gap-4">
                  ...
                  {!inDetailReviewTemplate && (
                    <button
                      className="text-center  py-3 mt-[10px] mb-[18px] rounded-md border-0 uppercase w-full bg-[#26BADA] text-white"
                      onClick={() => {
                        addDetailRevRow();
                        setshowDetailTempSelect(false);
                        setDetailTemplate(false);
                  ...
                  <div className="rounded-md border border-gray-300 w-full mt-4 ">
                    ...
                    <div className="bg-[#F8FAFB] text-center p-8">
                      <p className="text-5xl text-[#778CA2]">
                        {/*'17 '*/} : <span className="text-[#6DD230]">{/*'Compiled'*/}</span>
                      </p>
                      <p className=" text-[#778CA2] my-4">
                  ...
                  <div className="flex flex-col-reverse">
                    <div className="border mb-3 rounded-md ">
                      <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-between ">
                        ...
                        <p>
                          Posted by{" "}
                          <span className="text-[#00AAEC]">Ravi K.</span> on 23
                          Jun 2021, 12:30
                        </p>
                      </div>
                      <div className="bg-[#F4F5F6] p-6 flex flex-col gap-5">
                        <p>
                          Hi Michael,22 Please find enclosed the first set of
                          commercial questions. Can you please get
                    ...
                    <div className="border mb-3 rounded-md ">
                      ...
                      <div className="bg-white p-6 flex flex-col gap-5">
                        <p>
                          Hi Ravi Please find attached the updated file with
                          commercial clarifications from the customer.
                          <br />
                          Regards, Michael
                        </p>
                        <p className="text-[#778CA2] mt-3">
              ...
              <>
                <div className="flex justify-between uppercase text-[#00AAEC] text-sm mb-4 cursor-pointer ">
                  <div
                    ...
                    onClick={() => {
                      setClarificationDetail(true);
                      setShowSubmitedRfxCalrification(false);
                      setSelectedContacts([]);
                    }}
                  >
                  ...
                  <button
                    ...
                    onClick={handleChangeStatus}
                    disabled={!rfxClarRows.length ? true : false}
                  >
                    Proceed <FaArrowRight />{" "}
                  </button>
                </div>
                ...
                <div className="flex flex-[2] flex-col p-4 gap-6">
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <TextField
                      ...
                      className="bg-white w-full max-w-[360px]"
                      onChange={handleChangeRfxClarifRefNum}
                    ...
                    <textarea
                      onChange={(e) =>
                        setclarificationDescription(e.target.value)
                      }
                      className="w-full border border-[#E8ECEF] outline-none p-2 rounded-md"
                      placeholder="Description"
                      rows={4}
                ...
                <div className="flex-[1]">
                  ...
                  <div className="border mb-3 rounded-md">
                    <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-between items-center">
                      <span>Assign to</span>
                      <span
                        className="text-[#00AAEC] text-sm uppercase flex gap-1 items-cente cursor-pointer"
                    ...
                    <BidDialog
                      openBid={openContactAssign}
                      handleBidClose={handlCloseContact}
                      onPersonSelect={handlePersonSelectDailog}
                  ...
                  <div>
                    ...
                    <ContactDialog
                      isOpen={isContactDialogOpen}
                      handleClose={() => setContactDialogOpen(false)}
                      handleContactSelect={handleClarifContactSelect}
                      users={allUsersRec}
                    />
                  </div>
                </div>
            ...
            {showFinalRewList && (
              <>
                <div className="flex justify-between uppercase text-[#00AAEC] text-sm mb-4 cursor-pointer ">
                  ...
                  <button
                    className={`upprecase uppercase max-w-[200px] rounded-md p-2 flex items-center gap-1  ${
                      finalRevrows.length > 0
                        ? "bg-[#00AAEC] text-white"
                        : "bg-[#EFF3F5] text-[#98A9BC]"
                    }`}
                    onClick={handleChangeStatus}
                    disabled={finalRevrows.length > 0 ? false : true}
                  >
                    Proceed <FaArrowRight />{" "}
                  </button>
                </div>
                <SearchTableNew
                  rows={finalRevrows}
                  handleRowClick={handleRowClick}
                  NoRowsOverlay={NoRowsFinalReview}
                />
              </>
            )}
            {showFinalTempSelect && !finalTemplate && (
              <div className="flex flex-col gap-4 mt-5">
                <div
                  className=" flex items-center gap-1 text-[#00AAEC] cursor-pointer"
                  onClick={() => {
                    setFinalTemplate(false);
                    setshowFinalTempSelect(false);
                    setShowFinalRewList(true);
                  }}
                >
                  <FaChevronLeft />
                  <span> Back to List</span>
              ...
              <div className="flex">
                <div className="flex-[3]">
                  ...
                  <div>
                    ...
                    <div className="mt-4 dyn-template">
                      <div
                        dangerouslySetInnerHTML={{ __html: templateHTMLFinal }}
                      />
        ...
        {active === "Prepration" && (
          <Prepration handleChangeStatus={handleChangeStatus} />
        )}
            ...
            {showSubmissionTable && (
              <>
                <div className="flex justify-between uppercase text-[#00AAEC] text-sm mb-4 cursor-pointer ">
                  ...
                  <button
                    className={`upprecase uppercase max-w-[200px] rounded-md p-2 flex items-center gap-1 mb-4 mr-3  ${
                ...
                <SearchTableNew
                  rows={submissionrows}
                  handleRowClick={handleRowClick}
                  NoRowsOverlay={NoRowsSubmission}
                />
              </>
            )}
                  ...
                  <div className="border mb-3 rounded-md">
                    <div className="border mb-3 rounded-md">
                      ...
                      <div className="bg-[#F4F5F6] py-3 px-4 flex   items-center justify-between">
                        <AssignPopupInput
                          label={"Assign to"}
                          setBidSubAssignto={setBidSubAssignto}
                          allUsersRec={allUsersRec}
                          rfx_bid_assignto={rfxRecord.rfx_bid_assignto}
                          rfxID={rfxRecord.rfx_id}
                          className="w-full"
              ...
              <div className="flex gap-6">
                <div className="flex flex-[3] flex-col">
                  ...
                  <p className="text-xl p-2">
                    {selectedSubmissionRow.bid_type}
                  </p>
                  <div className="flex flex-col-reverse">
                    <div className="border mb-3 rounded-md ">
                      <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-between ">
                        ...
                        <p>
                          ...
                          on {formatDatetime(selectedSubmissionRow.created_on)}
                        </p>
                      </div>
                      <div className="bg-[#F4F5F6] p-6 flex flex-col gap-5">
                    ...
                    <div className="">
                      ...
                      <div className="p-4">
                        <textarea
                          ...
                          onChange={(e) =>
                            setSubmissionReplyText(e.target.value)
                          }
                ...
                <div className="flex-[2]">
                  <div className="border mb-3 rounded-md">
                    ...
                    <div className="bg-[#c3eff92f] px-4 py-1 flex flex-col  ">
                      ...
                      <div className="flex items-center justify-between w-full p-4">
                        <div>
                          <span className="text-[#778CA2]">Issued Date</span>{" "}
                          <br />
                  ...
                  <div className="border mb-3 rounded-md">
                    ...
                    {submissionAcknowledgement &&
                      !submissionAcknowledgement.acknowledged && (
                        <div className="bg-[#F4F5F6] py-6 flex items-center flex-col gap-3">
                          ...
                          <p className="text-[#00AAEC] cursor-pointer">
                    ...
                    {submissionAcknowledgement &&
                      submissionAcknowledgement.acknowledged && (
                        <div className="bg-[#F4F5F6] py-6 flex items-center flex-col gap-3">
                          <p>
                            {submissionAcknowledgement.acknowledgement_comment}
                          </p>
                          <div className="flex flex-[3] bg-white border rounded-[30px] p-1 gap-2 items-center max-w-[60%] w-full ">
                            <Image
                              src="/man.jpeg"
                            ...
                            <div className="">
                              <span className="text-sm leading-4 w-8">
                                {submissionAcknowledgement.user?.first_name}{" "}
                                {submissionAcknowledgement.user?.last_name}
                              </span>
                              <span className="text-sm leading-4 text-[#778CA2] block">
                                {
                                  submissionAcknowledgement.user
                                    ?.designation_title
                                }
                              </span>
                            </div>
            ...
            {documentDetail && (
              <div className="flex p-8 ">
                <div className="flex-[3]">
                  <div className="border mb-3 rounded-md mt-4 ">
                    <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-between ">
                      ...
                      <p>
                        Submitted by{" "}
                        <span className="text-[#00AAEC]">Bryan C.</span> on 20
                        Jul 2021, 12:30
                      </p>
                    </div>
                    <div className="bg-[#F8FAFB] px-6 py-8 flex flex-col gap-5">
                      <p>
                        Hi Michael, Please find enclosed the Tehcnical proposal.
                      </p>
                      <p className="text-[#778CA2] mt-3">Attached Documents</p>
                  ...
                  <div className="">
                    ...
                    <div className="flex flex-col">
                      <div className="flex items-center gap-1 my-3">
                        ...
                        />
                        <p className="bg-[#F2F4F6] px-4 py-2 rounded-full max-w-[80%] pr-[20%]">
                          Hi Bryan, Thanks for the timely submission.{" "}
                        </p>
                        <span className="uppercase text-[#98A9BC] text-xs">
                          08:00 PM
                        </span>
                      ...
                      <div className="flex items-center flex-row-reverse gap-1 my-3">
                        ...
                        />
                        <p className="bg-[#98A9BC] text-white px-4 py-2 rounded-full max-w-[80%] pr-[20%]">
                          You are welcome Michael. Thanks{" "}
                        </p>
                        <span className="uppercase text-[#98A9BC] text-xs">
                          09:30 PM
                        </span>
                      ...
                      <div className="flex items-center gap-1 my-3">
                        ...
                        />
                        <p className="bg-[#F2F4F6] px-4 py-2 rounded-full max-w-[80%] pr-[20%]">
                          Hopefully customer will get back with clarifications
                          soon{" "}
                        </p>
                        <span className="uppercase text-[#98A9BC] text-xs">
                          09:45 PM
                      ...
                      {/* If date changes */}
                      <p className="text-[#778CA2] flex items-center gap-10 my-8 w-[50%] m-auto after:content-[''] after:bg-slate-400 after:w-full after:h-[0.5px] after:border before:content-[''] before:bg-slate-400 before:w-full before:h-[0.5px] before:border">
                        Today
                      </p>
                      <div className="flex items-center gap-1 my-3">
                        <Image
                        ...
                        />
                        <p className="bg-[#F2F4F6] px-4 py-2 rounded-full max-w-[80%] pr-[20%]">
                          Hi Bryan, The clarifications are now posted. Thanks{" "}
                        </p>
                        <span className="uppercase text-[#98A9BC] text-xs">
                          10:10 PM
                        </span>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex justify-between">
                        ...
                        />
                        <button className="text-white border border-[#26BADA] bg-[#26BADA] uppercase text-sm px-8 py-3 min-w-[200px] rounded-sm ">
                          Add
                        </button>
                      </div>
                    </div>
                ...
                <div className="flex-[2] px-5">
                  ...
                  <div className="border mb-3 rounded-md">
                    <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] ">
                      Status <span className="text-black">Issued</span>
                    </div>
                    <div className="bg-[#c3eff92f] px-4 py-1 flex flex-col  ">
                      <div className="flex items-center justify-between w-full p-4">
                        <div>
          ...
          <div className="p-5">
            {!showBidClarificationDetial && (
              <>
                <div className="flex justify-end">
                  <button
                    className={`upprecase uppercase max-w-[200px] mb-3 rounded-md p-2 flex items-center gap-1  ${
                      submissionrows
                        ? "bg-[#00AAEC] text-white"
                        : "bg-[#EFF3F5] text-[#98A9BC]"
                    }`}
                    onClick={handleChangeStatus}
                    disabled={!submissionrows}
                  >
                    Proceed <FaArrowRight />{" "}
                  </button>
                </div>
                <SearchTableNew
                  handleRowClick={handleRowClick}
                  rows={bidClarifRows}
                  NoRowsOverlay={NoRowsOverlayBidClarification}
                />
              </>
            )}
            ...
            {showBidClarificationDetial && (
              <div className="flex gap-6">
                <div className="flex flex-[3] flex-col p-5">
                  <div
                    className=" flex items-center gap-1 text-[#00AAEC] cursor-pointer"
                    onClick={handleClickBidClarifList}
                  >
                    <FaChevronLeft />
                    <span> Back to List</span>
                  </div>
            ...
            {showRevisionTable && (
              <div className="justify-between">
                <div className=" width-[100%] justify-between uppercase text-[#00AAEC] text-sm mb-4 px-3">
                  <button
                    className=" flex items-center gap-1 upprecase uppercase max-w-[200px] rounded-md p-2 bg-[#00AAEC] text-white"
                    onClick={handleChangeStatus}
                    disabled={false}
                  >
                    Proceed <FaArrowRight />{" "}
                    ...
                    <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-between ">
                      ...
                      {bidOrderSelectedRow.acknowledged && (
                        <span className="flex items-center">
                          ...
                          {formatDatetime(bidOrderSelectedRow.acknowledged_on)}
                        </span>
                      )}
                    </div>
                    <div className="bg-[#F4F5F6] py-6 flex items-center flex-col gap-3">
                      ...
                      <OrderAcknowledgement
                        open={openOrderAck}
                        handleClose={handleOrderAckClose}
                        onYesClick={OrderAckYesClick}
                        handleTextChange={handleDailogTextChange}
                        textValue={DailogtextValue}
                        rfxID={bidOrderSelectedRow.rfx_id}
                        tenantID={tenantID}
                        apiBackendURL={apiBackendURL}
                        login_user_id={login_user_id}
                        dailogTitle={"Order Acknowledgement"}
                        orderRow={bidOrderSelectedRow}
                        bidOrderAcknowledgementRow={setBidOrderSelectedRow}
                      />
                    </div>
                  </div>
                </div>
        ...
        {(active === "Order Handover" || active === "Close") && (
          <div>
            {!showOrderDone && (
              <div className="flex gap-4">
                <div className="flex-[2] p-5">
                  ...
                  <p className="text-[#778CA2] my-2 text-lg">
                    Attached Documents
                  </p>
                  <div className="my-4">
                    <p className="text-[#778CA2] my-1 flex gap-1 items-center">
                      Purchase Order <IoMdAddCircleOutline />
                    </p>
                    {closingOrderDocvalt &&
                      closingOrderDocvalt
                  ...
                  <div className="my-4">
                    <p className="text-[#778CA2] my-1 flex gap-1 items-center">
                      Bid Documents <IoMdAddCircleOutline />
                    </p>
                    {closingOrderDocvalt &&
                      closingOrderDocvalt
                        .filter((doc) =>
                          doc.docvalt_key.includes("bid-submission")
                        )
                        .map((doc, index) => (
                          <div className=" shadow-sm flex items-center w-full p-2 justify-between mb-3 border-b border-[#babec2]">
                            <div className="flex items-center gap-4">
                  ...
                  <div className="my-4">
                    ...
                    {closingOrderDocvalt &&
                      closingOrderDocvalt
                        .filter((doc) =>
                          doc.docvalt_key.includes("rfx-clarifications")
                        )
                        .map((doc, index) => (
                          <div className=" shadow-sm flex items-center w-full p-2 justify-between mb-3 border-b border-[#babec2]">
                            <div className="flex items-center gap-4">
                  ...
                  <div className="my-4">
                    ...
                    {closingOrderDocvalt &&
                      closingOrderDocvalt
                        .filter((doc) =>
                          doc.docvalt_key.includes("bid-clarifications")
                        )
                        .map((doc, index) => (
                          <div className=" shadow-sm flex items-center w-full p-2 justify-between mb-3 border-b border-[#babec2]">
                            <div className="flex items-center gap-4">
                  ...
                  <div className="border mt-[18px] mb-3 rounded-md">
                    ...
                    <div className="bg-[#F4FCFD] px-4 py-5 flex items-center justify-between ">
                      <div>
                        <span className="text-[#778CA2] block">Issue Date</span>
                        <span>
                          {formatDateString(closingOrderDetails?.issued_date)}
                        </span>
                      </div>
                      <div>
                        <span className="text-[#778CA2] block">Due Date</span>
                        <span>
                          {formatDateString(
                            closingOrderDetails?.acknowledgement_deadline
                          )}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#F4FCFD] px-4 py-5">
                      <span className="text-[#778CA2] block">
                        Order Acknowldegement Date
                      </span>
                      <span>
                        {formatDateString(
                          closingOrderDetails?.acknowledgement_date
                        )}
                      </span>
                    </div>
                  </div>
