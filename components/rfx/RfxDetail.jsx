...
const RfxDetail = ({
  ...
  const [stages, setStages] = useState(
    [
     {stage:"rfxstage", displayName: "Initiate", newName: "", status: "current", order: 1},
     {stage:"rfxstage", displayName: "Initiate2", newName: "", status: "", order: 2}
   ]  
 );
 ...
 const [overviewData, setoverviewData] = useState([
   ...
   { name: "RFx Record Type", value: rfxRecord.rfx_record_type ?? "" },
   { name: "Previous RFx Reference", value: rfxRecord.prev_rfx_reference ?? "" },
   ...
   { name: "Bid Type", value: rfxRecord.bid_type ?? "" },

   { name: "Customer Framework Agreement", value: rfxRecord.work_agreement_ref_num ?? "" },
   { name: "Bid Submission Mode", value: rfxRecord.bid_submission_mode ?? "" },

   { name: "Other Submission Instructions", value: rfxRecord.submission_instructions ?? "" },
   { name: "Submission Contents", value: rfxRecord.submission_contents ?? "" },
   ...
   { name: "Opportunity Number", value: data.opp_number ?? "" },
   { name: "Opportunity Name", value: data.opp_title ?? "" },
       
   { name: "End User Project", value: rfxRecord.enduser_project ?? "" },

   { name: "Customer", value: rfxRecord.customer_name ?? "" },
   { name: "Sales Persuit Progress", value: rfxRecord.opp_pursuit_progress ?? "" },

   { name: "End User ", value: rfxRecord.enduser_name ?? "" },
   { name: "Opportunity Type", value: rfxRecord.opp_type ?? "" },

   { name: "Region", value: rfxRecord.region ?? "" },
   { name: "Opportunity Industry", value: rfxRecord.opp_industry ?? "" },

   { name: "Business Line", value: rfxRecord.opp_business_line ?? "" },
   { name: "Project Type", value: rfxRecord.project_type ?? "" },

   { name: "Opportunity Value", value: rfxRecord.opp_value ?? "" },
   { name: "Opportunity Currency", value: rfxRecord.opp_currency ?? "" },
   
   { name: "Bidding Unit", value: rfxRecord.bidding_unit ?? "" },
   { name: "Opportunity Committed for Sales Budget", value: rfxRecord.opp_comm_sales_budget ?? "" },
       
   { name: "Description", value: rfxRecord.description ?? "" },
 ]);
              ...
              <div className="flex-[1] flex flex-col">
                ...
                <div className="border mt-[18px] mb-3 rounded-md">
                  ...
                  <div className="bg-[#F4FCFD] px-4 py-5 ">
                    ...
                    <span>
                      {rfxRecord.issue_date
                        ? formatDateString(rfxRecord.issue_date)
                        : "No Date"}
                    </span>
                  </div>
                  ...
                  <div className="bg-[#F4FCFD] px-4 py-5">
                    ...
                    <span>
                      {rfxRecord.tech_clarif_deadline
                        ? formatDateString(
                            rfxRecord.tech_clarif_deadline
                          )
                        : "No Date"}
                  ...
                  <div className="bg-[#F4FCFD] px-4 py-5">
                    ...
                    <span>
                      {rfxRecord.comm_clarif_deadline
                        ? formatDateString(rfxRecord.comm_clarif_deadline)
                        : "No Date"}
                    </span>
                  </div>
                </div>
